...
class Authenticate:
    ...
    def _update_config_file(self):
        try:
            ...
            # Update the credentials part of the config
            ...
            print("Config file updated successfully.")  # For debugging, remove in production
            st.session_state['show_register'] = False
        except Exception as e:
            print(f"Error updating config file: {e}")  # For debugging, remove in production
    ...
    def register_user(self, location: str='main', preauthorization: bool=True, domains: Optional[list]=None, 
        ...
        new_password_repeat = register_user_form.text_input('Repeat password' if 'Repeat password' not in fields else fields['Repeat password'],
                                                             type='password')
        new_role = register_user_form.radio('Role' if 'Role' not in fields else fields['Role'],
                                        ['Student', 'Educator'])
        r_b = register_user_form.form_submit_button('Register' if 'Register' not in fields else fields['Register'])
        if r_b:
